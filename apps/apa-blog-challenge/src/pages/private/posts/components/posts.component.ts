...
export class PostsComponent implements OnInit {
  ...
  constructor(
    ...
    this.filterForm.valueChanges.subscribe(({ filter }) => {
      if (!!filter) {
        this.postsFiltered = this.postsData.filter(
          (post: any) =>
  ...
  public onCreateOrEditSuccess({ save = false, created = false }) {
    if (save) {
  ...
  public async openDetailPost(postId: any) {
    this.detailPost = await this.postsService.getPostById(postId);
