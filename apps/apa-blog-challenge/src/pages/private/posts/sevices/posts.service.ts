...
export class PostsService {
    ...
    public async getPostsById(id: number) {
        return await this.postsServiceRemote.getPostsById(id);
    }

    public async updatePost(id: any = 101, userId: any = 101, title: string, body: string) {
    ...
    public async setActivePost(post: any) {
        if (!!post?.userId) {
            const {username}: any = await this.usersService.getUserById(post?.userId);
